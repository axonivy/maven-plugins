...
/**
 * Generates a list of tables. The tables are sorted in the way that the content
 * of the table can be deleted in this order without to break any references.
 * @author rwei
 * @since 22.10.2009
 */
public class SortedTableContentDeleteListGenerator extends AbstractSortedTableContentListGenerator {
  ...
  public void generateMetaOutput(SqlMeta metaDefinition) throws Exception {
    ...
    try {
      List<SqlTable> tables = new TablesSortedByDeleteOrder(new DbHints(fDatabaseSystem), metaDefinition)
              .byDeleteOrder();
