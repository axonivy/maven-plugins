...
public class OracleSqlScriptGenerator extends SqlScriptGenerator {
  ...
  /**
   * System Database Hint ConvertEmptyStringToNull: Columns are generated with
   * NULL instead of NOT NULL. Persistency layer will write "" values as NULL.
  ...
  @Override
  public void generateAlterTableAlterColumn(PrintWriter pr, SqlTableColumn newColumn, SqlTable table,
    ...
    if (newColumn.isCanBeNull() != oldColumn.isCanBeNull()) {
      ...
      GenerateAlterTableUtil.generateAlterTableAlterColumnNotNull(pr, this, newColumn, table, "MODIFY",
              "NOT NULL", "NULL");
      changed = true;
    }
    if (!changed) {
    ...
    List<SqlUpdateColumnExpression> columnExpressions = Arrays.asList(
            new SqlUpdateColumnExpression(varcharColumn.getId(),
                    new SqlFunction("to_char",
                            new SqlFullQualifiedColumnName(table.getId(), numberColumn.getId()))));
  ...
  protected void generateDefaultValue(PrintWriter pr, SqlTableColumn column) {
    if (alterTable) {
      if (column.getDefaultValue() != null && "".equals(column.getDefaultValue().getValue())
              && !column.isCanBeNull()) {
        pr.append(" DEFAULT ");
  ...
  /**
   * @see ch.ivyteam.db.meta.generator.internal.SqlScriptGenerator#generateAlterTableDropUniqueConstraint(java.io.PrintWriter,
   *      ch.ivyteam.db.meta.model.internal.SqlTable,
   *      ch.ivyteam.db.meta.model.internal.SqlUniqueConstraint, java.util.List)
   */
  @Override
