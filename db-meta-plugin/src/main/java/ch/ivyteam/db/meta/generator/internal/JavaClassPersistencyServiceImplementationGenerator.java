...
/**
 * A Meta data generator that creates persistency service implementation classes
 * for entity java classes
 * @author rwei
 * @since 16.10.2009
 */
public class JavaClassPersistencyServiceImplementationGenerator extends JavaClassGenerator
        implements IMetaOutputGenerator {
  ...
  public JavaClassPersistencyServiceImplementationGenerator() {
    options.addOption(Option.builder().desc("Package where the entity classes are located.").required()
            .hasArg().longOpt(OPTION_ENTITY_PACKAGE).build());
  }
  ...
  private void writeJavaClassPersistencyServiceImplementationGenerator(SqlView view, SqlMeta meta)
          throws Exception {
  ...
  private void writeJavaClassPersistencyServiceImplementationGenerator(SqlTable table, SqlMeta meta)
          throws Exception {
