...
public class ReplaceOldTriggerVariable
{
  ...
  public SqlDmlStatement replace(SqlDmlStatement stmt)
  {
    ...
    else if (stmt instanceof SqlUpdate)
    {
      ...
      return new SqlUpdate(update.getTable(), update.getColumnExpressions(), replace(update.getFilterExpression()), update.getDatabaseManagementSystemHints(), update.getComment());
    }
  ...
  private SqlSimpleExpr replace(SqlSimpleExpr filterExpression)
  {
    if (filterExpression instanceof SqlBinaryRelation)
    {
      ...
      return new SqlBinaryRelation(replace(binRel.getFirst()), binRel.getOperator(), replace(binRel.getSecond()));
    }
    else if (filterExpression instanceof SqlLogicalExpression)
    {
      ...
      return new SqlLogicalExpression(replace(logicalExpr.getFirst()), logicalExpr.getOperator(), logicalExpr.getSecond());
    }
  ...
  private SqlAtom replace(SqlAtom expression)
  {
    ...
    else if (expression instanceof SqlCaseExpr)
    {
      ...
      return new SqlCaseExpr(replace(caseExpr.getColumnName()), replaceInWhenThenList(caseExpr.getWhenThenList()));
    }
    else if (expression instanceof SqlComplexCaseExpr)
    {
      SqlComplexCaseExpr caseExpr = (SqlComplexCaseExpr) expression;
      return new SqlComplexCaseExpr(replaceInComplexWhenThenList(caseExpr.getWhenThenList()), replace(caseExpr.getElseAction()));
    }
  ...
  private SqlFullQualifiedColumnName replace(SqlFullQualifiedColumnName columnName)
  {
    ...
    {
      return triggers.dmlStatements.identifiers.replaceTriggerVariable(columnName, triggers.getRowTriggerOldVariableName());
    }
