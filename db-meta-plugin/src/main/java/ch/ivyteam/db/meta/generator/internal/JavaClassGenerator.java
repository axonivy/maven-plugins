...
public abstract class JavaClassGenerator implements IMetaOutputGenerator {
  ...
  protected final Options options = new Options()
          .addOption(Option.builder().desc("Output root directory for the generated classes.").required()
                  .hasArg().longOpt(OPTION_OUTPUT_DIR).build())
          .addOption(Option.builder().desc("Package name of the created java classes.").required().hasArg()
                  .longOpt(OPTION_PACKAGE).build())
          .addOption(Option.builder().desc("Tables, the classes has to be generated for").required().hasArgs()
                  .longOpt(OPTION_TABLES).build());
  ...
  /**
   * Class name databse system hint. Specifies the class name of the java
   * entitiy class generated for the table
   */
  public static final String CLASS_NAME = "ClassName";
  /**
   * Secondary keys database system hints. Use this to specify list of columns
   * that build the secondary keys
   */
  public static final String SECONDARY_KEYS = "SecondaryKeys";
  /**
   * Parent can be modified database system hint. Marks that the parent key can
   * be modified
   */
  ...
  /**
   * Password database system hint. Marks that the column value has to be
   * encrypted in the database
   */
  public static final String PASSWORD = "Password";
  /**
   * Truncate database system hint. Marks that values have to be truncated
   * before writting into the databse
   */
  public static final String TRUNCATE = "Truncate";
  /**
   * DataType database system hint. Marks that the given java data type should
   * be used instead the default one
   */
  public static final String DATA_TYPE = "DataType";
  /**
   * Database system hint. Indicates, that the database field is of type 'int'
   * and in java an enumeration is used with the given name. The defined
   * enumeration must implement the interface IPersistentEnumeration
   */
  public static final String ENUM = "Enum";
  /**
   * Attribute name database system hint. Specifies the attribute name of the
   * java entity class generated for the column
   */
  public static final String ATTRIBUTE_NAME = "AttributeName";
  /**
   * Additional set methods database system hint. Specifies the column name that
   * should be set with the additional set method
   */
  public static final String ADDITIONAL_SET_METHODS = "AdditionalSetMethods";
  /**
   * As Association database system hint. Specifies that Association constants
   * should be generated in the foreign tables of this association table
   */
  public static final String AS_ASSOCIATION = "AsAssociation";
  /**
   * Query table name system hint. Specifies the table name that should be used
   * for reading entity data
   */
  public static final String QUERY_TABLE_NAME = "QueryTableName";
  /**
   * query field name system hint. Specifies the name of a column in a generated
   * query class. If not set the name of the column is used as field name
   */
  public static final String QUERY_FIELD_NAME = "QueryFieldName";
  /**
   * Specifies that this field should not appear as queryable field on the
   * generated Query (e.g. CaseQuery)
   */
  public static final String HIDE_FIELD_ON_QUERY = "HideFieldOnQuery";
  /**
   * <p>
   * Specifies the filter query data type of a column in a generated query
   * class. If not set the data type of the column is used to evaluate the as
   * field name.
   * </p>
   * <p>
   * Most often used to keep the old filter query data type when a data type of
   * a column changes so that the query interface is still compatible with older
   * versions.
   * </p>
   */
  ...
  /**
   * Field is deprecated and should no longer be used. Instead use the field
   * given as value
   */
  ...
  /**
   * Table has legacy security member fields, defined in either Role or User
   * table.
   */
  ...
  protected void analyseAdditionalArgs(@SuppressWarnings("unused") CommandLine commandLine)
          throws Exception {}
   ...
   */
  protected void writeColumnInformation(PrintWriter pr, SqlTable table, SqlTableColumn column)
          throws MetaException {
  ...
  /**
   * Gets the associations tables where the given table is reference as foreign
   * key
  ...
  protected List<SqlTable> getAssociationTables(SqlTable table, SqlMeta metaDefinition) {
    ...
    for (SqlTable associationTable : metaDefinition.getArtifacts(SqlTable.class)) {
      if (associationTable.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
              .isHintSet(AS_ASSOCIATION)) {
        ...
        if (associationTable.getColumns().get(0).getReference() == null) {
          throw new MetaException(
                  "No reference defined on the column " + associationTable.getColumns().get(0).getId()
                          + " of association table " + associationTable.getId());
        }
        if (associationTable.getColumns().get(1).getReference() == null) {
          throw new MetaException(
                  "No reference defined on the column " + associationTable.getColumns().get(1).getId()
                          + " of association table " + associationTable.getId());
        }
