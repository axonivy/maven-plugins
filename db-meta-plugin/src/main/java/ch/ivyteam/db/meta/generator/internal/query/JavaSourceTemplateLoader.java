...
class JavaSourceTemplateLoader implements TemplateLoader {
  ...
  public Reader getReader(Object templateSource, String encoding) throws IOException {
    ...
    if (template.indexOf(startTag) < 0) {
      // tag not found inside source file -> return empty string so that nothing
      // gets included
      System.err.println("Start tag '" + startTag + "' not found in template file "
              + source.getLeft().getAbsolutePath());
