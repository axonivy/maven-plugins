...
public class JavaQueryClassGenerator extends JavaClassGenerator {
  ...
  public JavaQueryClassGenerator() {
    options.addOption(Option.builder().desc("Template directory (e.g. for Public API)").required().hasArg()
            .longOpt(OPTION_TEMPLATE_DIR).build());
    options.addOption(Option.builder().desc("Source directory (e.g. for java source templates)").required()
            .hasArg().longOpt(OPTION_SOURCE_DIR).build());
  }
  ...
  protected void analyseAdditionalArgs(CommandLine commandLine) throws Exception {
    ...
    if (!templateDir.exists()) {
      throw new IllegalArgumentException(
              "Template directory '" + templateDir.getAbsolutePath() + "' does not exist");
    }
    sourceDir = new File(commandLine.getOptionValue(OPTION_SOURCE_DIR));
    if (!sourceDir.exists()) {
      throw new IllegalArgumentException(
              "Source directory '" + sourceDir.getAbsolutePath() + "' does not exist");
    }
  }
  ...
  private void writeJavaQueryClass(SqlMeta meta, SqlTable table) throws MetaException, IOException {
    JavaQueryClassTemplateWriter javaQueryClassTemplate = new JavaQueryClassTemplateWriter(meta, table,
            getTargetPackage(), templateDir, sourceDir);
