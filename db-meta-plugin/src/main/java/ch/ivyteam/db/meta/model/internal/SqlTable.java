    ...
    for (SqlForeignKey foreignKey : fForeignKeys)
    {
      ...
      for (SqlTableColumn column : fColumns)
      {
        if (column.getId().equals(foreignKey.getColumnName()))
        {
          ...
          {
            throw new MetaException("Foreign key reference already declared for column '"+column.getId()+"' in table '"+getId()+"'");
          }
      ...
      if (!found)
      {
        throw new MetaException("Column '"+foreignKey.getColumnName()+"' specified in foreign key definition not defined in table '"+getId()+"'");
      }
    }
    for (SqlTableColumn column : fColumns)
    {
      if (column.getReference() != null)
      {
        ...
        if (!found)
        {
          fForeignKeys.add(new SqlForeignKey(null, column.getId(), column.getReference(), column.getDatabaseManagementSystemHints(), null));
        }
      }
    }
