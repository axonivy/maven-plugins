...
public final class JavaClassGeneratorUtil {
  ...
  public static String getJavaClassName(SqlTable table) {
    String className;
    if (table.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
            .isHintSet(JavaClassGenerator.CLASS_NAME)) {
      className = table.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
              .getHintValue(JavaClassGenerator.CLASS_NAME);
    } else {
  ...
  public static String getJavaClassName(SqlView view) {
    String className;
    if (view.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
            .isHintSet(JavaClassGenerator.CLASS_NAME)) {
      className = view.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
              .getHintValue(JavaClassGenerator.CLASS_NAME);
    } else {
  ...
  public static String generateJavaIdentifier(SqlTableColumn column) {
    if (column.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
            .isHintSet(JavaClassGenerator.ATTRIBUTE_NAME)) {
      return StringUtils.capitalize(column.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
              .getHintValue(JavaClassGenerator.ATTRIBUTE_NAME));
    } else {
      return column.getId();
    }
  }
  ...
  public static List<SqlTableColumn> getClobColumns(SqlTable table) {
    ...
    for (SqlTableColumn column : table.getColumns()) {
      if ((column.getDataType().getDataType() == SqlDataType.DataType.CLOB) &&
              (!"String".equals(column.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
                      .getHintValue(JavaClassGenerator.DATA_TYPE)))) {
        columns.add(column);
      }
    }
    return columns;
  ...
  /**
   * Gets the java data type of the given column. Data Type Java system hints
   * are considered.
   * @param column
   * @param noNativeDataTypes if no native data types like int, boolean, float
   *          should be returned. Instead Integer, Boolean, Float will returned
   * @return java data type
   */
  protected static String getJavaDataType(SqlTableColumn column, boolean noNativeDataTypes) {
    if (column.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
            .isHintSet(JavaClassGenerator.DATA_TYPE)) {
      return column.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
              .getHintValue(JavaClassGenerator.DATA_TYPE);
    } else if (column.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
  ...
  /**
   * Gets a list of all columns of the table without the primary and parent key
   * columns and all columns with LOB data type
  ...
  public static String getParentKey(SqlTable table) {
    for (SqlForeignKey foreignKey : table.getForeignKeys()) {
      if (foreignKey.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
              .isHintSet(JavaClassGenerator.AS_PARENT)) {
  ...
  public static String getFieldForOptimisticLocking(SqlTable table) {
    for (SqlTableColumn col : table.getColumns()) {
      if (col.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
  ...
  public static boolean isPasswordColumn(SqlTableColumn column) {
    return column.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
            .isHintSet(JavaClassGenerator.PASSWORD);
  }

  public static SqlTableColumn getOptionalPasswordColumnFor(SqlTable table, SqlTableColumn column) {
    String optionalPasswordColumnName = column.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
            .getHintValue(JavaClassGenerator.PASSWORD);
    ...
    if (optionalPasswordColumn == null) {
      throw new MetaException("Optional password control column " + optionalPasswordColumnName
              + " defined on column " + column.getId() + " not found in table " + table.getId());
    }
  ...
  public static SqlView getQueryView(SqlMeta metaDefinition, SqlTable table) {
    if (!table.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
            .isHintSet(JavaClassGenerator.QUERY_TABLE_NAME)) {
      return null;
    }
