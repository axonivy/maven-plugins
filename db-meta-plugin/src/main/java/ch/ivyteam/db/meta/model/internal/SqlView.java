...
public class SqlView extends SqlObject {
  ...
  /**
   * The selects used to define the view. If more than one select exists combine
   * them with UNION ALL.
   */
    ...
    for (SqlSelect select : fSelects) {
      if (columns.size() > select.getExpressions().size()) {
        throw new MetaException(
                "Missing expression(s) in view " + id + ". There are more columns defined than expressions");
      } else if (columns.size() < select.getExpressions().size()) {
        throw new MetaException(
                "Missing column(s) in view " + id + ". There are more expression defined than columns");
      }
    }
  }
