...
/**
 * This class contains information to generate java code depending on a
 * {@link SqlTableColumn}.
 * @author fs
 * @since 11.01.2012
 */
public abstract class ColumnInfo {
  ...
  private static List<ColumnInfo> getColumns(TableInfo tableInfo) {
    ...
    for (SqlTableColumn column : tableInfo.getTable().getColumns()) {
      if (!column.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
              .isHintSet(JavaClassGenerator.HIDE_FIELD_ON_QUERY)) {
  ...
  private static List<ColumnInfo> getColumns(SqlMeta meta, TableInfo tableInfo, SqlView view) {
    ...
    for (SqlViewColumn column : view.getColumns()) {
      if (!column.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
              .isHintSet(JavaClassGenerator.HIDE_FIELD_ON_QUERY)) {
        result.add(new ViewColumnInfo(meta, view.getTableAliases(), tableInfo, column,
                view.getSelects().get(0).getExpressions().get(pos).getExpression()));
      }
      pos++;
    }
    return result;
  ...
  private DataType getDataType() {
    if (getColumn().getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
            .isHintSet(JavaClassGenerator.FILTER_QUERY_DATA_TYPE)) {
      return DataType.valueOf(getColumn().getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
              .getHintValue(JavaClassGenerator.FILTER_QUERY_DATA_TYPE));
    }
  ...
  public boolean isEnumeration() {
    return getColumn().getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
            .isHintSet(JavaClassGenerator.ENUM);
  }

  public EnumerationInfo getEnumerationInfo() {
    String enumName = getColumn().getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
  ...
  public boolean isDeprecated() {
    return getColumn().getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
            .isHintSet(JavaClassGenerator.DEPRECATED);
  }

  public String getDeprecatedUseColumnInstead() {
    String useColumnNameInstead = getColumn().getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
            .getHintValue(JavaClassGenerator.DEPRECATED);
    if (StringUtils.isBlank(useColumnNameInstead)) {
      throw new IllegalStateException(
              "No column defined which should be used instead of deprecated column " + getName());
    }
    SqlTableColumn useColumnInstead = tableInfo.getTable().findColumn(useColumnNameInstead);
    if (useColumnInstead == null) {
      throw new IllegalStateException("Column '" + useColumnNameInstead
              + "' defined to used instead of deprecated column " + getName() + " not found");
    }
  ...
  private static class ViewColumnInfo extends ColumnInfo {
    ...
    public ViewColumnInfo(SqlMeta meta, Map<String, String> tableAliases, TableInfo tableInfo,
            SqlViewColumn column, SqlAtom expression) {
      super(tableInfo);
      this.meta = meta;
    ...
    @Override
    public String getName() {
      String id = column.getDatabaseManagementSystemHints(JavaClassGenerator.JAVA)
        ...
        if (!getTableInfo().getTable().getId().equals(columnName.getTable())) {
          return JavaClassGeneratorUtil.convertToJavaDoc(
                  "<p>This is a virtual column. It contains the same value as the column " +
                          ...
                          JavaClassGeneratorUtil.removeTablePrefix(columnName.getTable()) + "</code>.</p>",
                  5);
        }
      }
      return null;
