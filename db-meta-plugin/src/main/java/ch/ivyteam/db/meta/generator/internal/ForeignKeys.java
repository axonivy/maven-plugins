...
public class ForeignKeys
{
   ...
   * the method {@link #generate(PrintWriter, SqlForeignKey)} will be called after the column generation.</br>
  ...
  /**
   * Generates the foreign key definition. This method is only called if method {@link #isReferenceInColumnDefinitionSupported()} 
   * returns false.
  ...
  public void generateAlterTableDrop(PrintWriter pr, SqlTable table, SqlForeignKey foreignKey, @SuppressWarnings("unused") List<String> createdTemporaryStoredProcedures)
  {
    pr.print("ALTER TABLE ");
    identifiers.generate(pr, table.getId());
